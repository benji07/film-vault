...
import { filmName, filmIso } from "@/utils/film-helpers";

export function FilmRow({ film, onClick, cameras }) {
        ...
        <div className="flex gap-1.5 mt-1 flex-wrap">
          ...
          {cam && (
            <Badge style={{ color: T.green, background: T.green + "18" }}>
              {cam.name}{back ? ` · ${back.name}` : ""}
            </Badge>
          )}
          {isExpired && <Badge style={{ color: T.accent, background: T.accent + "18" }}>Périmée</Badge>}
          {isExpiring && !isExpired && <Badge style={{ color: T.amber, background: T.amber + "18" }}>Expire bientôt</Badge>}
        </div>
      </div>
